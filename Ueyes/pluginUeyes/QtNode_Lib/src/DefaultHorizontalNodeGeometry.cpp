...
QPointF DefaultHorizontalNodeGeometry::buttonPosition(NodeId const nodeId) const
{
    QSize size = _graphModel.nodeData<QSize>(nodeId, NodeRole::Size);

    unsigned int captionHeight = captionRect(nodeId).height();

    if (auto button  = _graphModel.nodeData<QWidget  *>(nodeId, NodeRole::Button)) {
        // If the widget wants to use as much vertical space as possible,
        // place it immediately after the caption.
        if (button->sizePolicy().verticalPolicy() & QSizePolicy::ExpandFlag) {
            return QPointF(2.0 * _portSpasing + maxPortsTextAdvance(nodeId, PortType::In),
                           captionHeight);
        } else {
            return QPointF(size.width() - button->width() - 10, 10);
        }
    }
    return QPointF();
}
