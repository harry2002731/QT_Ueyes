...
NodeId DataFlowGraphModel::addNode(QString const nodeType,QMainWindow & debugWin)
{
    std::unique_ptr<NodeDelegateModel> model = _registry->create(nodeType);
    model->setDebugWin(debugWin);

    if (model) {
        NodeId newId = newNodeId();

        // 数值若发生改变则触发，会把链接的节点都进行更新
        connect(model.get(),
    ...
    NodeId nodeId, PortType portType, PortIndex portIndex, QVariant const &value, PortRole role)
{
    ...
    case PortRole::Data:
        if (portType == PortType::In) {
            ...
            Q_EMIT inPortDataWasSet(nodeId, portType, portIndex); // 触发信号
        }
        break;
