...
void NodeGraphicsObject::embedQWidget()
{
    ...
    if (auto b = _graphModel.nodeData(_nodeId, NodeRole::Button).value<QWidget *>())
    {
        _proxyWidget2 = new QGraphicsProxyWidget(this);
        _proxyWidget2->setWidget(b);
        _proxyWidget2->setPos(geometry.buttonPosition(_nodeId));
...
void NodeGraphicsObject::mouseMoveEvent(QGraphicsSceneMouseEvent *event)
{
    ...
    if (_nodeState.resizing()) {
        ...
        if (auto b = _graphModel.nodeData<QWidget *>(_nodeId, NodeRole::Button)) {
            AbstractNodeGeometry &geometry = nodeScene()->nodeGeometry();
            _proxyWidget2->setPos(geometry.buttonPosition(_nodeId));
        }
    } else {
